...
export default function AIChat() {
  ...
  =========================== */
  useEffect(() => {
    localStorage.setItem("incog_ai_chats", JSON.stringify(chatHistory));
  }, [chatHistory]);
  ...
  function deleteChat(chatId) {
    const updated = chatHistory.filter(item => item.id !== chatId);
  ...
  async function sendMessage() {
    ...
    try {
      const reply = await chatWithAssistant(question, history);
    ...
    return sections.map((section, index) => {
      ...
      lines.forEach((line, i) => {
        ...
        if (trimmed.startsWith("MATH:")) {
          const latex = trimmed.replace(/^MATH:/, "").trim();
          elements.push(
            <div
              key={`math-${i}`}
              className="incog-display-math"
              dangerouslySetInnerHTML={{
                __html: convertLatexToMarkup(latex, { displayMode: true })
              }}
            />
          );
          return;
        ...
        if (
          ...
          elements.push(
            <h3 key={`title-${i}`} className="incog-section-title">
              {trimmed}
            </h3>
          );
          return;
        ...
        if (trimmed.startsWith("•")) {
          elements.push(
            <li key={`bullet-${i}`} className="incog-chat-bullet">
        ...
        // Normal paragraph
        elements.push(
          <p key={`text-${i}`} className="incog-chat-paragraph">
            {trimmed}
          </p>
        );
      });
      ...
      return (
        <div key={index} className="incog-chat-section">
          {elements}
        </div>
      );
    });
  ...
  return (
    <div className="incog-chat">
      {/* ==========================
          SIDEBAR
      ========================== */}
      <aside className={sidebarOpen ? "incog-sidebar open" : "incog-sidebar"}>
        <div className="incog-sidebar-header">
          <button className="incog-new-chat" onClick={createNewChat}>
            + New Chat
          </button>
          <button
        ...
        <div className="incog-history">
          {filteredChats.length === 0 ? (
            <div className="incog-empty-history">No conversations</div>
          ) : (
      ...
      <div className="incog-main">
        <header className="incog-header">
          <button className="incog-menu" onClick={() => setSidebarOpen(true)}>
            ☰
          </button>

          <div className="incog-header-title">INCOG AI</div>

          <button className="incog-clear" onClick={clearCurrentChat}>
            Clear
          </button>
        </header>

        <div className="incog-chat-history" ref={chatRef}>
            ...
            >
              <div className="incog-message-content">
                {message.role === "assistant" ? (
                  ...
                  <p className="incog-chat-paragraph">{message.content}</p>
                )}
              </div>

              <div className="incog-message-footer">
                <span className="incog-message-time">{message.time}</span>
        ...
        <div className="incog-disclaimer">
          INCOG can make mistakes. Verify important calculations.
        </div>
        ...
        <div className="incog-chat-input">
          ...
          >
            {loading ? <span className="incog-loading">Thinking...</span> : "Send"}
          </button>
        </div>
